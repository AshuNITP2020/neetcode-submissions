class Solution {
    public boolean isValidSudoku(char[][] board) {
        boolean isAllRowValid = isRowValid(board, true);
        boolean isAllColumnValid = isRowValid(board, false);
    ...
    boolean isRowValid(char[][] board, boolean isRow) {
        for (int i = 0; i < 9; i++) {
            List<Character> list = new ArrayList<>();
            for (int col = 0; col < 9; col++) {
                char ch = isRow ? board[i][col] : board[col][i];
